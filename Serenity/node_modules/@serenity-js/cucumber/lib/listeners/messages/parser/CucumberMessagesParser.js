...
class CucumberMessagesParser {
    constructor(serenity, formatterHelpers, formatterOptionsAndDependencies, shouldReportStep) {
    ...
    parseTestCaseFinishes(hookMessage) {
        return new events_1.SceneFinishes(this.serenity.currentSceneId(), this.currentScenario, this.outcomeFrom(hookMessage.result), this.serenity.currentTime());
    }
    ...
    scenarioHierarchyTagsFor(gherkinDocument, pickle) {
        ...
        return notEmpty([
            themeName && new model_1.ThemeTag(themeName),
            capabilityName && new model_1.CapabilityTag(capabilityName),
    ...
    parseTestCaseAttempt(testCaseAttempt) {
        ...
        testCaseAttempt.testCase.testSteps.forEach(step => {
            if (!testCaseAttempt.stepResults[step.id]) {
                testCaseAttempt.stepResults[step.id] = { status: messages_1.messages.TestStepFinished.TestStepResult.Status.UNKNOWN };
            }
        });
        // ---
    ...
    activityDetailsFor(parsedTestStep) {
        return new model_1.ActivityDetails(new model_1.Name(this.testStepFormatter.format(parsedTestStep.keyword, parsedTestStep.text, parsedTestStep.argument)));
    }
    outcomeFrom(worstResult, ...steps) {
        const Status = messages_1.messages.TestStepFinished.TestStepResult.Status;
        switch (worstResult.status) {
            ...
            case Status.FAILED:
                const error = io_1.ErrorSerialiser.deserialiseFromStackTrace(worstResult.message);
    ...
    scenarioOutcomeFrom(testCaseAttempt) {
        ...
        const tags = testCaseAttempt.attempt > 0
            ? [new model_1.ArbitraryTag('retried'), new model_1.ExecutionRetriedTag(testCaseAttempt.attempt)]
            : [];
        return { outcome, tags };
    }
}
