...
module.exports = {
    ...
    handler: (argv) => {
        ...
        core_1.configure({
            actors: new stage_1.UpdateCommandActors(new io_1.Path(process.cwd()), () => io_2.axiosClient(repository, !!argv.ignoreSSL, process.env, model_1.Credentials.fromString(argv.auth))),
            crew: [
                new stage_1.NotificationReporter(printer),
                new stage_1.ProgressReporter(printer),
            ],
        });
        return Promise.resolve().then(() => core_1.actorCalled('Serenity/JS Updater').attemptsTo(core_1.Check.whether(screenplay_1.FileExists.at(pathToArtifact), assertions_1.isTrue())
            ...
            .catch(error => {
            printer.error(io_2.formatError(error));
